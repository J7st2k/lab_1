...
{
    this->setWindowTitle("Счетчик");  //формимрование Gui
    ...
    QHBoxLayout *layout1 = new QHBoxLayout(); //логика та же, что и в 2.7
    ...
    edit1->setEnabled(false); //отключим возможность менять значения в счётчике вручную
...
void Counter::add_one()
{
    QString str=text();
    int r=str.toInt();
    if (r!=0 && (r+1)%5 ==0) emit tick_signal();
    r++;
    str.setNum(r);
    setText(str);
}
